...
Contrastive learning is an SSL technique that produces meaningful representations (embeddings) from an unlabelled dataset. The key idea of contrastive learning is to contrast semantically similar (positive) and dissimilar (negative) pairs of data points, encouraging the representations of positive pairs to be close together in the embedding space, and those of negative pairs to be more orthogonal \cite{chuang2020debiased}. This can be done in a single-modal or multi-modal (cross-modal) setting, where by modality we refer to the type of data input such as images or text. 
...
\label{sec:downstream}
 Since the aim of InfoNCE is to bring the vector representations of the same object close together, it has the result of creating a structured embedding space where semantically similar objects are close together, while semantically dissimilar objects are far apart. We can therefore view the embedding space as a multi-dimensional space of point-objects with distances between them having semantic meaning. Therefore, the embeddings are useful on their own be useful for a variety of tasks. We refer to these tasks that come after the training process as \textbf{downstream tasks}. Downstream tasks on the embedding space can include similarity searches or even prediction of physical properties of the objects purely based on the space structure. We refer to the later as \textbf{zero-shot} predictions, as the model has not been trained on the specific task. 
...
In AstroCLIP, we consider two modalities of galaxies: images and spectra. In essence, we approach the two modalities as filtered, noisy views of the same galaxy. To obtain their  embeddings, we deploy a pair of models to encode the images and spectra. This process is carried out in two steps:
\begin{enumerate}
    ...
    \item We append a simple Multilayer Perceptron (MLP) to each of the pre-trained models to compress the representations into $d=128$-dimensions. We then proceed to train (or fine-tune) both MLP projection heads jointly under InfoNCE loss, to align the embeddings of the two modalities under shared semantics.
\end{enumerate}
...
To detect these islands on the UMAP projection, we use the DBSCAN (Density-Based Spatial Clustering of Applications with Noise) clustering algorithm \cite{dbscan} from \verb|scikit-learn|. DBSCAN is an unsupervised machine learning algorithm designed to identify clusters in large spatial datasets by examining the local density of data points. The algorithm categorises points into core points, border points, and noise points, governed by two parameters: $\epsilon$ (epsilon) and $\text{minPts}$ (minimum points). These are defined as follows:
\begin{itemize}
...
Fig.\ref{fig:umap_of_images} and Fig.\ref{fig:umap_of_both} show the UMAP projection of the image and the combined image-spectrum embeddings, respectively. Similarly to the spectrum embeddings, we again see a clear structuring, with one difference: these mappings include a large `island' of galaxies that does not contain particular property values. Rather, it largely follows a similar pattern to the main `island', indicating this break-off is caused by some other feature.

To further examine this and other potential islands, we use DBSCAN to directly cluster the UMAP projection of the image embeddings. We set $\epsilon = 0.20$ and the minimum number of samples to 5. These were chosen by trial and error to produce a reasonable number of clusters in both the image and spectrum projection, with a reasonable number of galaxies in each. We then draw sample galaxies for each cluster to visually inspect. The clusters for the image embeddings are illustrated in Fig.\ref{fig:umap_DBSCAN}.

The clustering reveals that the image embedder has successfully separated galaxy images that contain artifacts caused by the image capture process to a distinct island (Cluster 2). The galaxies contained in those images have variable redshifts and stellar masses, and follow the same distribution as the main island. This is potentially a useful ability of the model to quickly identifying artifacts in galaxy catalogues. We further display 3 more islands: cluster 3 and 4 contain galaxies that largely fill the image, are mostly spiral and have high brightness. This indicates that the model is able to separate galaxies based on their morphology, which is a key feature in galaxy classification. 

It is important to note that although we visualise image embeddings in isolation, their structure is informed by the spectra during training, which means that it contains information about the galaxy's spectrum as well. This is also true for the spectrum embeddings, which are informed by the images. We make this statement more concrete by showcasing the results of $k$-Means clustering on the full dimensional space of just the spectrum embeddings. Here, we first perform the clustering and then project the clusters onto the UMAP projection. We choose to use 10 clusters, as they yield a reasonable silhouette score and a reasonable number of galaxies in each cluster. 
...
We then visualise a random sample of the spectra in each cluster, as shown in Fig.\ref{fig:annotated_spectra}. We observe that galaxies in the same cluster follow similar trends in their spectra, both in terms of the overall shape, the presense of specific spectral features and the flux range and scale. In addition to this, we show that not only are the spectra in the same cluster similar, but the images of the galaxies are also visually similar, as shown in Fig.\ref{fig:annotated_images}. Recall that we are only considering the spectrum embeddings in this case, which means that the model has learned to structure them in a way that is informed by the images. This is a key feature of the AstroCLIP model, as it allows for the alignment of the two modalities in the shared latent space. Furthermore, it reveals a potential capability of the model to cluster galaxies based on morphology, which is a key feature in galaxy classification. 

A noteworthy feature of this latest UMAP projection is a distinct ring of galaxies on the main island that is permeating into different clusters. This could be an interesting pattern to investigate further, as it could be indicative of a specific physical property that is not captured by the redshift or stellar mass.
